...
**Note on Original Indication:** No Taiwan/NZ license or indication text exists in the regulatory data provided (`market_status: Not marketed`, 0 licenses). The original-indication context above (schizophrenia/bipolar disorder) reflects general public drug knowledge, not data extracted from this evidence pack, and should be confirmed against a formal source before use in any regulatory submission.
